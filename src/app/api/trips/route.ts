...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = await sql`
      SELECT id, lake_slug, activity, description, planned_date, group_size, join_count, created_at, expires_at
      FROM trips
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { lake_slug, activity, description, planned_date, group_size } = body;
    ...
    const sql = getSQL();
    const rows = await sql`
      INSERT INTO trips (lake_slug, activity, description, planned_date, group_size, expires_at)
      VALUES (${lake_slug}, ${activity}, ${description || null}, ${planned_date}, ${safeGroupSize}, ${planned_date}::date + INTERVAL '1 day')
